  ...
  return (
    ...
    >
      <View style={styles.thumbContainer}>
        ...
        {/* Liked badge — reflects the PC's rating (>=5) or a local favorite */}
        {isLiked && (
...
export default function LibraryScreen() {
  ...
  const renderItem = useCallback(({ item }: { item: MediaItem }) => (
    <GridItem
      ...
      // Liked if the PC rates it >=5 or it's a local favorite.
      isLiked={(item.rating ?? 0) >= 5 || isFavorite(item.id)}
    />
  ), [itemWidth, handleItemPress, handleItemLongPress, handleThumbLoadError, handleDownload, downloads, isFavorite])
