...
export default function PlaylistsScreen() {
  ...
  const submitCreate = async () => {
    ...
    try {
      ...
      Alert.alert('Create playlist failed', err?.message ?? 'Failed')
    } finally {
      setCreating(false)
    }
  }
  ...
  const handlePlayPress = (playlist: Playlist) => {
    ...
    router.push(`/playlist/${playlist.id}?autoplay=1` as any)
  }
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Create-playlist FAB (always available) */}
      <TouchableOpacity
        style={styles.fab}
        onPress={() => {
          if (Platform.OS === 'ios') Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium)
        ...
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            ...
            <View style={styles.modalBtns}>
              <TouchableOpacity style={styles.modalCancel} onPress={() => setCreateVisible(false)} disabled={creating}>
                <Text style={styles.modalCancelText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.modalCreate} onPress={submitCreate} disabled={creating || !newName.trim()}>
...
const styles = StyleSheet.create({
  ...
  modalCard: { backgroundColor: '#18181b', borderRadius: 16, padding: 20, borderWidth: 1, borderColor: '#27272a' },
  modalTitle: { color: '#fafafa', fontSize: 18, fontWeight: '700', marginBottom: 14 },
  modalInput: { backgroundColor: '#09090b', borderRadius: 10, color: '#fafafa', paddingHorizontal: 14, paddingVertical: 12, fontSize: 16, borderWidth: 1, borderColor: '#3f3f46' },
  modalBtns: { flexDirection: 'row', gap: 10, marginTop: 16 },
  modalCancel: { flex: 1, paddingVertical: 12, borderRadius: 10, backgroundColor: '#27272a', alignItems: 'center' },
  modalCancelText: { color: '#a1a1aa', fontSize: 15, fontWeight: '600' },
  modalCreate: { flex: 1, paddingVertical: 12, borderRadius: 10, backgroundColor: '#3b82f6', alignItems: 'center' },
