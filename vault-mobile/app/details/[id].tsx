...
export default function MediaDetailsScreen() {
  ...
  const openPlaylistPicker = async () => {
    ...
    try {
      ...
      // Manual playlists only (can't add to smart/rule-based ones).
      setPlaylists((res.items || []).filter((p: any) => !p.isSmart))
    } catch (err) {
      Alert.alert('Playlists', getErrorMessage(err))
    } finally {
  ...
  const addToPlaylist = async (playlistId: string, playlistName: string) => {
    ...
    try {
      ...
      Alert.alert('Add to playlist failed', getErrorMessage(err))
    } finally {
      setAddingTo(null)
    }
  }
  ...
  const handleScan = async () => {
    ...
    setIsScanning(true)
    try {
      await api.enqueueMedia([id!])
      Alert.alert('Queued for AI scan', 'Sent to the PC to be analyzed. It will appear in AI Review → Pending when done.')
    } catch (err) {
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.actionsRow}>
        ...
        <TouchableOpacity style={styles.actionButton} onPress={openPlaylistPicker}>
        ...
        <View style={styles.pickerBackdrop}>
          <View style={styles.pickerSheet}>
            ...
            {loadingPlaylists ? (
              ...
              <Text style={styles.pickerEmpty}>No playlists yet. Create one on the Sessions tab.</Text>
            ) : (
              <ScrollView style={{ maxHeight: 360 }}>
                {playlists.map((p) => (
                  <TouchableOpacity key={p.id} style={styles.pickerRow} onPress={() => addToPlaylist(p.id, p.name)} disabled={addingTo === p.id}>
                    <Ionicons name="musical-notes" size={18} color="#3b82f6" />
                    <Text style={styles.pickerRowText} numberOfLines={1}>{p.name}</Text>
                    {addingTo === p.id
                      ? <ActivityIndicator size="small" color="#3b82f6" />
                      : <Text style={styles.pickerRowCount}>{p.itemCount}</Text>}
                  </TouchableOpacity>
                ))}
              </ScrollView>
            )}
...
const styles = StyleSheet.create({
  ...
  pickerSheet: { backgroundColor: '#18181b', borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20, paddingBottom: 34 },
  pickerHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 },
  pickerTitle: { color: '#fafafa', fontSize: 18, fontWeight: '700' },
  pickerEmpty: { color: '#71717a', fontSize: 14, textAlign: 'center', paddingVertical: 24 },
  pickerRow: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: '#27272a' },
  pickerRowText: { color: '#fafafa', fontSize: 15, flex: 1 },
  pickerRowCount: { color: '#52525b', fontSize: 13 },
