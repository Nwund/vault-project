...
export default function PlayerScreen() {
  ...
  // Add-to-playlist from the watch screen
  const [playlistPickerVisible, setPlaylistPickerVisible] = useState(false)
  const [pickerPlaylists, setPickerPlaylists] = useState<Array<{ id: string; name: string; itemCount: number }>>([])
  ...
  const openPlaylistPicker = useCallback(async () => {
    ...
    try {
      ...
      setPickerPlaylists((res.items || []).filter((p: any) => !p.isSmart))
    } catch (err) {
  ...
  const addToPlaylistFromPlayer = useCallback(async (playlistId: string, name: string) => {
    setAddingToPlaylist(playlistId)
    try {
      await api.addToPlaylist(playlistId, [currentId])
  ...
  return (
    <GestureHandlerRootView style={styles.container}>
                ...
                <>
                  ...
                  <View style={styles.topBar}>
                    ...
                    <View style={styles.topBarRight}>
                      ...
                      <TouchableOpacity style={styles.iconButton} onPress={openPlaylistPicker}>
      ...
      <Modal visible={playlistPickerVisible} transparent animationType="slide" onRequestClose={() => setPlaylistPickerVisible(false)}>
        <View style={styles.plPickerBackdrop}>
          <View style={styles.plPickerSheet}>
            <View style={styles.plPickerHeader}>
              <Text style={styles.plPickerTitle}>Add to playlist</Text>
            ...
            {loadingPlaylists ? (
              ...
              <Text style={styles.plPickerEmpty}>No playlists yet. Create one on the Sessions tab.</Text>
            ) : (
              <ScrollView style={{ maxHeight: 360 }}>
                {pickerPlaylists.map((p) => (
                  <TouchableOpacity key={p.id} style={styles.plPickerRow} onPress={() => addToPlaylistFromPlayer(p.id, p.name)} disabled={addingToPlaylist === p.id}>
                    <Ionicons name="musical-notes" size={18} color="#3b82f6" />
                    <Text style={styles.plPickerRowText} numberOfLines={1}>{p.name}</Text>
                    {addingToPlaylist === p.id
                      ? <ActivityIndicator size="small" color="#3b82f6" />
                      : <Text style={styles.plPickerRowCount}>{p.itemCount}</Text>}
                  </TouchableOpacity>
                ))}
              </ScrollView>
            )}
...
const styles = StyleSheet.create({
  ...
  plPickerBackdrop: { flex: 1, backgroundColor: 'rgba(0,0,0,0.7)', justifyContent: 'flex-end' },
  plPickerSheet: { backgroundColor: '#18181b', borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20, paddingBottom: 34 },
  plPickerHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 },
  plPickerTitle: { color: '#fafafa', fontSize: 18, fontWeight: '700' },
  plPickerEmpty: { color: '#71717a', fontSize: 14, textAlign: 'center', paddingVertical: 24 },
  plPickerRow: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: '#27272a' },
  plPickerRowText: { color: '#fafafa', fontSize: 15, flex: 1 },
  plPickerRowCount: { color: '#52525b', fontSize: 13 },
  title: {
    flex: 1,
    color: '#fff',
    fontSize: 16,
