...
function LibraryPage(props: { settings: VaultSettings | null; selected: string[]; setSelected: (ids: string[]) => void; tagBarOpen: boolean; setTagBarOpen: (open: boolean | ((prev: boolean) => boolean)) => void; confetti?: ReturnType<typeof useConfetti>; anime?: ReturnType<typeof useAnime> }) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const startIndex = (currentPage - 1) * LIBRARY_PAGE_SIZE
      const endIndex = Math.min(startIndex + LIBRARY_PAGE_SIZE, sortedMedia.length)
  ...
  }, [focusedIndex, sortedMedia, LIBRARY_PAGE_SIZE, currentPage, getColumnsCount, openIds, addFloatingPlayer])
      ...
      <div className="h-[calc(100vh-110px)] flex min-w-0 relative">
        ...
        <div ref={contentRef} className="flex-1 overflow-auto p-4 transition-all duration-300 ease-in-out">
          ...
          <div className="flex items-center justify-end mb-4 px-1">
            {sortedMedia.length > LIBRARY_PAGE_SIZE && (
              <div className="flex items-center gap-2">
                <button
                ...
                <span className="text-xs text-white/60">
                  Page {currentPage} of {Math.ceil(sortedMedia.length / LIBRARY_PAGE_SIZE)}
                </span>
                <button
                  onClick={() => setCurrentPage(p => Math.min(Math.ceil(sortedMedia.length / LIBRARY_PAGE_SIZE), p + 1))}
                  disabled={currentPage >= Math.ceil(sortedMedia.length / LIBRARY_PAGE_SIZE)}
                  className="px-2 py-1 rounded text-xs bg-white/5 hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed"
                >
                  Next →
                </button>
          ...
          <div ref={gridRef} style={getGridStyle()}>
            {sortedMedia.slice((currentPage - 1) * LIBRARY_PAGE_SIZE, currentPage * LIBRARY_PAGE_SIZE).map((m, index) => {
